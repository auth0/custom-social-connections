var Home = React.createClass({
	...
	componentWillMount: function() {
      ...
      this.setState({connections: data});
		...
		var onError    = function(xhr, err, st){
      console.log(err);
    };
    ...
    var connection = new Connection(),
      onComplete = function() {
        this.closeModal();
        this.setState({
          connections: this.removeConnection(id)
        });
      };
  ...
  getConnection: function(id) {
    return this.state.connections.filter(function(el) {
      return el.id === id;
    })[0];
  },
    ...
    return this.state.connections.filter(function(connection) {
	...
	render: function() {
		var that = this,
			...
			rows = this.state.connections.map(function (connection) {
				return (
          <AuthConnectionRow tryit={that.onTryIt} key={connection.id} openDialog={that.openModal} id={connection.id} name={connection.name} strategy={connection.strategy} />
        );
			}),
